...
export default function DashboardView({
  ...
  const { currentDate, setActiveTab, setReviewMode, setFlowTimerMinutes } =
    useAppStore();

  // 从本地数据库读取数据
  const habits = useLiveQuery(() => db.habits.toArray()) || [];
  const todayLogs =
    useLiveQuery(() => db.habitLogs.where({ date: currentDate }).toArray()) ||
    [];
  const todayRecord = useLiveQuery(() => db.dailyRecords.get(currentDate));

  // 查询今日到期需要复习的卡片数
  const overdueCardsCount =
    useLiveQuery(async () => {
      const now = new Date();
      return await db.cards.filter((c) => c.nextReview <= now).count();
    }) || 0;

  // 连续打卡天数（计算包含今天在内的连续有活跃记录的天数，包含常规习惯打卡与已完成 AI 任务排程）
  const streakDays =
    useLiveQuery(async () => {
      ...
      console.log('DEBUG [streakDays] - datesSet:', Array.from(datesSet));
      ...
      console.log('DEBUG [streakDays] - todayStr:', todayStr, 'yesterdayStr:', yesterdayStr, 'dates:', dates);

      if (!dates.includes(todayStr) && !dates.includes(yesterdayStr)) {
        console.log('DEBUG [streakDays] - No hit today or yesterday');
        return 0;
      }
      ...
      console.log('DEBUG [streakDays] - Calculated streak:', streak);
      return streak;
    }, [currentDate]) || 0;
  ...
  const [activeSpecializedHabit, setActiveSpecializedHabit] =
    useState<Habit | null>(null);

  // AI 每日行动排程状态
  const [isGeneratingTasks, setIsGeneratingTasks] = useState(false);
  const [aiTasksParticles, setAiTasksParticles] = useState<
    Record<string, any[]>
  >({});
  ...
  // 保存特化习惯打卡数据
  const handleSaveSpecializedLog = async (
    customData: any,
    imageUrl?: string,
  ) => {
  ...
  const handleStartReviewFlow = async () => {
    ...
    try {
      ...
      // 提取 AI 任务数据
      const aiTasks = todayRecord?.aiTasks || [];
      ...
      if (rate === 1) {
        setReviewQuestions([
          ...
          '回顾今日愿望：“' +
            wishStr +
            '”，既然已完成，明天的成长边界你打算推到哪里？',
          totalTasks > 0 && completedTasks < totalTasks
            ? `虽然你达成了全部习惯打卡，但 AI 为你精算派发的 ${totalTasks} 个任务中你漏掉了【${uncompletedTasks.map((t) => t.name).join('、')}】。这是因为任务有些畏难，还是哪里执行力不力？`
        ...
        setReviewQuestions([
          ...
          totalTasks > 0 && completedTasks < totalTasks
            ? `AI 精算出的 ${totalTasks} 个排程任务中，你还有【${uncompletedTasks.map((t) => t.name).join('、')}】未完成。这几项任务是被你的拖延所扼杀，还是你能量评分是虚报数据？`
  ...
  const toggleHabitLog = async (habitId: string) => {
    ...
    if (habit && habit.dependsOn) {
      ...
      if (!isParentLogged) {
        ...
        alert(
          `🔒 行为链受限：请先完成前置习惯【${parent?.name || '前置习惯'}】的打卡！`,
        );
        return;
      }
    }
    ...
    if (habit && habit.customFormType && habit.customFormType !== 'none') {
      ...
      if (isAlreadyLogged) {
        const existing = await db.habitLogs
          .where({ habitId, date: currentDate })
          .first();
        if (existing) {
      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-border-subtle pb-6">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-1">
            你已经连续打卡{' '}
            <span className="text-primary font-bold">{mounted ? streakDays : 0}</span> 天 🔥
            继续保持！
          </p>
        </div>
          ...
          <section className="space-y-3">
            ...
            {todayRecord && todayRecord.woopWish ? (
              <SpotlightCard
                ...
                className="p-5 relative group overflow-hidden"
              >
            ...
            {habits.length === 0 ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-[9px] text-text-secondary uppercase tracking-widest font-mono">
                            ...
                            {habit.customFormType &&
                            habit.customFormType !== 'none'
                              ? '📋 特化表单'
                              : '常规打卡'}
                          </span>
                        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {sortedHabits.map((habit) => {
                  ...
                  const isRecommended =
                    habit.energyDemand === recommendedDemand && !isLogged;

                  // 前置依赖校验
                  const parentHabit = habit.dependsOn
                    ? habits.find((h) => h.id === habit.dependsOn)
                    : null;
                  const isLocked = parentHabit
                    ? !loggedHabitIds.has(parentHabit.id)
                    : false;
                      ...
                      <div className="w-full h-full flex items-center justify-between p-4">
                        ...
                        <div className="flex items-center justify-center flex-shrink-0 ml-3">
                          <div className="relative flex items-center justify-center w-6 h-6">
                            {/* 局部打卡 Confetti 特效 */}
                            {habitParticles[habit.id] &&
                              habitParticles[habit.id].map((p) => (
                                <motion.div
                                  key={p.id}
            ...
            <div className="z-10 space-y-3 mt-4">
              {isGeneratingTasks ? (
                ...
                <div className="space-y-4 py-2">
                  ...
                  <p className="text-[10px] text-text-secondary font-mono animate-pulse">
                    AI 教练正在审阅您近7日打卡趋势，准备行为习惯拆解...
                  </p>
                </div>
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span
                                className={`text-[8px] font-bold font-mono px-1.5 py-0.5 rounded uppercase tracking-wider ${
                              ...
                              <p
                                className={`text-xs font-bold text-text-primary ${isCompleted ? 'line-through text-text-secondary' : ''}`}
                              >
                                {task.name}
                              </p>
                            </div>
                          ...
                          <div className="flex-shrink-0 relative w-5 h-5 flex items-center justify-center mt-0.5">
                            {/* 局部打卡粒子特效 */}
                            {aiTasksParticles[task.id] &&
                              aiTasksParticles[task.id].map((p) => (
                                <motion.div
                                  key={p.id}
                                  className="absolute w-1 h-1 rounded-full pointer-events-none z-10"
                                  style={{ backgroundColor: p.color }}
                ...
                <div className="py-4 text-center space-y-4">
                  <p className="text-xs text-text-secondary leading-relaxed max-w-[280px] mx-auto">
                    今日尚未生成排程任务。AI Coach 将融合今日能量值及过去 7
                    天打卡惯性为您智能派发定制任务。
                  </p>
                  <button
                    type="button"
            ...
            {isReviewing ? (
              ...
              <div className="mt-4 space-y-3.5 text-xs text-text-secondary">
                ...
                <div className="space-y-2.5 bg-surface-2/60 p-3 rounded-lg border border-border-subtle">
                  {todayRecord.reviewQuestions?.map((q, idx) => (
                    <div key={idx} className="space-y-1">
                      <p className="text-text-primary font-bold font-mono text-[10px]">
                        Q: {q}
                      </p>
                      <p className="text-neutral-gray italic pl-2.5 border-l border-primary/30 text-[11px]">
                        A: {todayRecord.reviewAnswers?.[idx]}
                      </p>
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-4 space-y-3">
                <p className="text-xs text-text-secondary leading-relaxed">
                  每晚 21:00 提取今日打卡和 WOOP 状态，发送给大模型，生成 3
                  个犀利的个性化反思问题推送。
                </p>
                <button
                  onClick={handleStartReviewFlow}
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    <button
                      ...
                      onClick={() => {
                        ...
                        setOutcome(
                          '代码跑通，功能闭环，获得满满的掌控感与成就感',
                        );
                        setObstacle('觉得逻辑繁杂，写一会就想切出去刷社交软件');
                        setPlan(
                          '如果我想切出去刷社交软件，我就深呼吸一次，强迫自己只留在编辑器里再写 3 行代码。',
                        );
                      }}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setObstacle(
                          '下班回到家觉得很疲惫，只想躺在沙发上刷视频',
                        );
                        setPlan(
                          '如果我躺在沙发上不想动，我就只穿上运动鞋在客厅站 2 分钟，即便不运动也行。',
                        );
                      }}
                      className="px-2.5 py-1 bg-surface-3 hover:bg-primary/20 hover:text-primary transition-all rounded-md text-[10px] font-bold text-text-secondary border border-border-subtle"
                    >
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setOutcome(
                          '彻底锁定今天学到的技术点，打破遗忘曲线，学习效率倍增',
                        );
                        setObstacle('看到卡片数量觉得有些累，产生拖延情绪');
                        setPlan(
                          '如果我产生拖延情绪，那么我先点开「开始复习」，只要求自己复习最前面 2 张卡片。',
                        );
                      }}
                      className="px-2.5 py-1 bg-surface-3 hover:bg-primary/20 hover:text-primary transition-all rounded-md text-[10px] font-bold text-text-secondary border border-border-subtle"
                    >
